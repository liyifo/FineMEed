...
class RSMed(nn.Module):
    def __init__(self, voc_size: list, ehr_adj, ddi_adj, dmc_adj, pmc_adj,
        ...
        self.patient_linear = torch.nn.Sequential(
            torch.nn.ReLU(), # can remove
            nn.Linear(3*embedding_dimension, embedding_dimension)
        ) 
    ...
    def forward(self, diag, proc, labtest, history):
        ...
        '''
        # 编码表征
        diagnoses_embeddings = self.diagnosis_embedding(torch.LongTensor(diag).to(self.device)) # (d_len) -> (d_len, dim)
        # 程序表征
        procedures_embeddings = []
        for index, p in enumerate(proc):
            procedure_embeddings = self.procedure_embedding(torch.LongTensor([p]).to(self.device)) # (1,p_len,dim)
            # attention
            procedure_embeddings = torch.softmax(self.p_attn_w(procedure_embeddings)@diagnoses_embeddings[index].unsqueeze(-1), dim=1)*procedure_embeddings # (1, p_len, 1) -> (1,p_len,dim)
            procedures_embeddings.append(torch.sum(procedure_embeddings, keepdim=False, dim=1)) # (1,dim)
        
        procedures_embeddings = torch.cat(procedures_embeddings, dim=0) # (d_len, dim)

        # 实验室测试信息
        labtests_embeddings = []
        for index, l in enumerate(labtest):
            labtest_embeddings = self.lab_embedding(torch.LongTensor([l[0]]).to(self.device)) # (1,l_len,dim)
            scale = torch.LongTensor([l[1]]).to(self.device).view(1, -1, 1) # (1, l_len, 1)
            labtest_embeddings = labtest_embeddings * scale
            # attention
            labtest_embeddings = torch.softmax(self.l_attn_w(labtest_embeddings)@diagnoses_embeddings[index].unsqueeze(-1), dim=1)*labtest_embeddings
            labtests_embeddings.append(torch.sum(labtest_embeddings, keepdim=False, dim=1)) # (1,dim)
        labtests_embeddings = torch.cat(labtests_embeddings, dim=0) # (d_len, dim)
        ...
        diagnoses_embeddings = torch.cat([diagnoses_embeddings, procedures_embeddings, labtests_embeddings], dim=1) #(d_len, 3*emb_dim)
        diagnoses_embeddings = self.patient_linear(diagnoses_embeddings)
